...
namespace DAL
{
    public sealed class DynamicQuery
    {
        ...
        public static string GetInsertQuery(string tableName, dynamic item)
        {
            ...
            string[] columns = props.Select(p => p).Where(p => !p.GetGetMethod().IsVirtual).Select(p => p.Name).Where(s => s != "ID").ToArray();

            var result  = string.Format("INSERT INTO {0} ({1}) VALUES (@{2})",
                                 tableName,
                                 string.Join(",", columns),
                                 string.Join(",@", columns));
            return result;
